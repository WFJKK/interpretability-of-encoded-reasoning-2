...
from gen_cups import ALL_SLOT_WORDS, CONDITIONS, CUPS, chain_metrics, parse_answer, parse_chain
...
def tokcheck(args):
    ...
    multi = [w for w in ALL_SLOT_WORDS if len(tok(" " + w, add_special_tokens=False)["input_ids"]) != 1]
    print(f"  slot words ({len(ALL_SLOT_WORDS)}) single-token with leading space:", not multi, multi)
